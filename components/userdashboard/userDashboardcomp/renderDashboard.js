...
function RenderDashboard({ type }) {
  ...
  return (
    <Box w={isLargerthan950 ? "80vw" : "100%"} h="100%">
      <Box
        paddingRight={5}
        display="flex"
        w="100%"
      ...
      >
        {!isLargerthan950 && (
          <Button
            ...
            color="#fff"
            onClick={(e) => {
              setOpendrawer((val) => true);
            }}
            bg={deg1color}
            _active={{ bg: deg1color }}
            boxShadow={`0px 0px 50px ${primarycolor}`}
            _selected={{ bg: deg1color }}
            as={IconButton}
            icon={<HamburgerIcon />}
          />
        )}
        {!isLargerthan950 && (
          <Mobilesidebar
            setProfileView={updateCurrentView}
            setDrawerState={handleOpenDrawer}
            ...
            {({ isOpen }) => (
              <>
                ...
                >
                  ...
                  <MenuItem onClick={logout} _focus={{ bg: deg3color }}>
                    {" "}
                    Logout
                  </MenuItem>
                </MenuList>
