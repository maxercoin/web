...
const testnet = "https://ropsten.infura.io/v3/26c8fdb8dc0e441eb8c365f2bacfaaae";
const mainnet = "https://mainnet.infura.io/v3/e9e73ef9cafe42bf97895bf6423b07e7";
const web3 = new Web3("https://mainnet.infura.io")   
const bscTestnetRpc = "https://data-seed-prebsc-1-s1.binance.org:8545/";
const erctestNetUSDT = "0xB404c51BBC10dcBE948077F18a4B8E553D160084";
const bscUsdt = "0x337610d27c682e347c9cd60bd4b3b107c9d34ddd";
const bscTestNet = "0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F";
const UsdrErc20 = "0x6EE856Ae55B6E1A249f04cd3b947141bc146273c"
...
export async function getAllBalances(address){
  ...
  const ethTokensContracts = [{name:"USDT",address:ethUSDTMainnet,decimals:18}]
  const tokens = networks.map(async (v)=>{
    if(v==='bsc'){
      const web = new Web3(bscTestnetRpc) 
      ...
      let tokensBalance = bscTokensContracts.map(async(v)=>{
        const tokenBal = await getBalancesOfTokens(address,v.address,v.decimals,web)

        return  {name:v.name,decimals:v.decimals,balance:tokenBal.balance,price:'1',contract:v.address,network:'Bep20'}
      })
      return [{name:"BSC",balance:innateToken,decimals:v.decimals,price:bscPrice,network:'Bep20'},...(await Promise.all(tokensBalance))]
    }else if(v==='eth'){    
        ...
        let tokensBalance = ethTokensContracts.map(async(v)=>{
          ...
          return  {name:v.name,decimals:v.decimals,balance:tokenBal.balance,contract:v.address,price:'1',network:'Erc20'}
        })
        return [{name:"ETH",balance:innateToken,decimals:v.decimals,price:ethPrice,network:'Erc20'},...(await Promise.all(tokensBalance))]
      }
...
export async function calcBNBPrice(){
  ...
  let amountOut;
  try {
      let router = await new web.eth.Contract( pancakeSwapAbi, pancakeSwapContract );
...
export async function calcETHPrice(){
  ...
  try {
      let router = await new web.eth.Contract( pancakeSwapAbi, pancakeSwapContract );
      amountOut = await router.methods.getAmountsOut(bnbToSell, [ETHTokenAddress ,USDTokenAddress]).call();
      amountOut =  web.utils.fromWei(amountOut[1]);
  } catch (error) {}
  if(!amountOut) return 0;
  return amountOut;
...
export async function getBalanceFromAPi(address){
  const chainIds = [{id:42,network:"eth"},{id:97,network:"bsc"}]
  const apiKey = process.env.COINS_API_KEY
  const returnedBalances = chainIds.map(async(v)=>{
    let apiresp = await axios.get(`https://api.covalenthq.com/v1/${v.id}/address/${address}/balances_v2/?&key=${apiKey}`)
...
export const sendtoken = async(privKey,tokencontract,amount,chain,tokenshort,receivingAddr,fromAddr,deci)=>{
  const chainnet = {
    erc20:{
      id:42,
      rpc:"https://ropsten.infura.io/v3/26c8fdb8dc0e441eb8c365f2bacfaaae"},
    bep20:{
          id:97,
...
if(tokenshort.toLowerCase() === 'eth' || tokenshort.toLowerCase() === 'bsc'){
  try{
   ...
   const builTx = await web.eth.accounts.signTransaction ({
    ...
    const contract = new web.eth.Contract(minABI, tokencontract,{from: fromAddr});
    const transferCt = await contract.methods.transfer(receivingAddr, value).encodeABI();
    const gasEst = await web.eth.estimateGas({from:fromAddr,toAddress:receivingAddr})
    ...
    let transaction = await web.eth.accounts.signTransaction(rawTransaction, privKey)
    let reciept = await web.eth.sendSignedTransaction(transaction.rawTransaction)
    return reciept
  }catch(e){
      return {
        'error':e,
...
let successful = (txHist.blockHash && txHist.blockNumber && txHist.transactionIndex ) ? true : false
...
return {successful,from_address,to_address,block_signed_at,tx_hash,block_height,value_quote}
}catch(e){
