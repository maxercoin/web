    ...
    if(req.method?.toLocaleLowerCase() === "post"){
        ...
        if(!email || !password){
         res.status(400).send("Kindly fill the require fields.");
            ...
            const getUser = await user.findOne({$or: [{ email }, { userName:email }]})
            if(getUser && (await  bcrypt.compare(password,getUser.password))){
            //    res.status(200).send("Authorized!")
               const {_id:id,activated,walletAddress,firstName,lastName,referrer,userName,email} = getUser
               const userdata = {firstName,lastName,referrer,userName,email}
            //    res.status(200).json({id,activated})
               generateCookieResponse(200, res, id, 'user',activated,walletAddress,userdata);
            }else{
