...
import { generateAccount } from '../../../../../backendUtils/wallet/functions';
 export default async function handler(req, res) {
    ...
    if(req.method?.toLocaleLowerCase() === "post"){
        ...
        try{
        const {userName,password,email,confirmPassword,firstName,lastName,ref} = req.body;
        if(!userName || !password || !email || !confirmPassword || !firstName || !lastName){
            res.status(403).send(`Please fill all the required fields`);
        }
        if(password != confirmPassword) res.status(400).send("Your password must be equal t confirm password.")
        ...
        const {address,keystoreJsonV3} = await generateAccount(walletAccountPassword)
        ...
        try{
            ...
            if(myrefData != null && myrefData){
                ...
                await referral.updateOne({_id:myrefData._id},{$set:{refferals}})
            }
          ...
          sendConfirmationEmail(toUser,id);
          generateCookieResponse(200, res, id, 'user',false,address);
