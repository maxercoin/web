 ...
 export default async function handler(req, res) { 
     ...
     if(req.method?.toLocaleLowerCase() === "get"){
         ...
         if(!login){
            ...
            const userexist = await user.findOne({$or:[{email:login},{userName:login}]})
            if(userexist){
                const userotpfield = await otp.findOne({email:userexist.email})
                const userotp = otpGenerator.generate(6, { upperCaseAlphabets: false, specialChars: false });
                ...
                if(!userotpfield){
                    try{
                    const createfield = await otp.create({email:userexist.email,otp:userotp})
                    const sendEmail = await sendConfirmationEmail(touser," ",msg)
                    res.status(200).send('Otp send to your email address')
                }catch(e){
                    console.log(e.message)
                    res.status(400).send('An error occured while sending email, kindly refresh the page and try again')
                }
                }else{
                    try{
                        const createfield = await otp.updateOne({email:userexist.email},{$set:{otp:userotp}})
                        const sendEmail = await sendConfirmationEmail(touser," ",msg)
                        res.status(200).send('Otp send to your email address')
                    }catch(e){
                        console.log(e)
            ...
            if(userexist){
                ...
                if(otpstored){
                    ...
                    if(storedotp != userotp){
                        res.status(400).send("Otp not correct, Kindly refresh and try again.")
                    }else{
                    const newPasswordHash = await bcrypt.hash(newPassword, 10);
                    const updateUserPassword = await user.updateOne({_id:userexist._id},{$set:{password:newPasswordHash}})
                    res.status(200).json({updateUserPassword,messsage:"Password updated successfully."}) 
                    }
                }else{
                    res.status(400).send("No otp found, kinly refresh the page and try again to request for new otp for the user.")
                }
            }else{
