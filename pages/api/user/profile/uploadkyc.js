    ...
    const kycstatuses = {
     ...
     if(req.method?.toLocaleLowerCase() === "post"){
         ...
         if(!idtype || !idNuber || !frontSide || !backSide){
            ...
            const {url:backUrl} = await cloudinary.uploader.upload(backSide, {
                upload_preset: 'mexercoin_Storage',
            });
            const addKyc = await kyc.create({owner:userid,ownerId:userid,Name:userowner.ownername,idNumber:idNuber,frontUrl,backUrl,status:kycstatuses[1]})
            console.log(addKyc);
