...
function Login() {
      ...
      if(!password || !login){
        ...
        if(logUser.status === 200){
          ...
          if(!logUser.data.active){
            ...
            sessionStorage.setItem(process.env.NEXT_PUBLIC_USER_AUTH_KEY,logUser.data.token)
            ...
            sessionStorage.setItem("userprofileinfo",JSON.stringify(logUser.data.userdata))
            goto('/profile')
          }
        }
        setIsLoading(v=>false)
  ...
  <Head>
        ...
        <meta name="description" content="Mexer coin cryptocurrency Investment Website Login Page" />
    ...
    {!reqActivateMsg && <Box mb='5' w={isLargerthan834 ?'60%':'90%'} padding='10' borderRadius='15px' boxShadow={`2px 2px 100px ${colors.primarycolor}`} mt='5' border='1px solid #fff' display='flex' flexDirection='column' placeItems='center' gap='5' >
    <Text  fontSize='1.5em' fontWeight='900' color='#fff'>LOGIN </Text>
    ...
    <Button isLoading={isLoading} onClick={loginUser} _hover={{bg:colors.secondarycolor,color:'#fff'}}>Login </Button>
    <Text _hover={{cursor:'pointer',color:colors.deg2color  }} onClick={(e)=>goto('/forgotpassword')} color='#fff'><u><i>Forgot Password</i></u></Text>
    ...
    {reqActivateMsg && <Text color='#fff'>Kindly proceed to your email and the link sent to activate your account. Thanks.</Text>}
    </Box>
    <Footercomponent />
    </>
  )
